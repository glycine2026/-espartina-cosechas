    ...
    for i, ctg in enumerate(ctgs):
        ...
        with st.expander(titulo_expander, expanded=(i == 0 and not completado)):
            ...
            pre_items = [
                ...
                ("Fecha carga", ctg.get("fecha", "")),
                ("CTG", pre.get("CTG", "")),
            ]
            chips = "".join(
                f'''<span style="display:inline-flex;align-items:center;gap:4px;background:#fff;
                border:1px solid #c5ddb5;border-radius:6px;padding:3px 10px;margin:3px;font-size:0.78rem;color:#1b3a1b">
                <span style="font-weight:600;color:#3B6D11">{k}</span>
                <span style="color:#6b6b6b">{v}</span></span>'''
                for k, v in pre_items if v
            )
            st.markdown(f'''
            <div style="background:linear-gradient(135deg,#f0f7e8,#e8f2e0);border:1px solid #c5ddb5;
                        border-radius:10px;padding:12px 16px;margin-bottom:16px">
              <div style="font-size:0.65rem;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;
                          color:#3B6D11;margin-bottom:8px">Datos precargados desde Monday</div>
            ...
            def monday_note(texto):
                st.markdown(
                    f'''<div style="font-size:0.7rem;color:#3B6D11;background:#f0f7e8;
                    border-radius:4px;padding:2px 7px;display:inline-block;margin-top:-6px;margin-bottom:4px">
                    ↑ Monday</div>''',
                    unsafe_allow_html=True
                )
            ...
            with c2:
                formularios[form_key]["Fecha"] = st.text_input(
                    "Fecha de carga", value=ctg.get("fecha",""), key=f"{form_key}_fecha")
                monday_note("Fecha")
            with c3:
                st.text_input("CTG", value=ctg_id, key=f"{form_key}_ctg_val", disabled=True)
                monday_note("CTG")
            ...
            with c4:
                ...
                formularios[form_key]["Código socio"] = st.selectbox("Código socio *", opts, index=idx, key=f"{form_key}_socio")
                monday_note("Código socio")
            with c5:
                ...
                formularios[form_key]["Código campaña"] = st.selectbox("Código campaña *", opts, index=idx, key=f"{form_key}_camp")
                monday_note("Código campaña")
            with c6:
                ...
                formularios[form_key]["Código especie"] = st.selectbox("Código especie *", opts, index=idx, key=f"{form_key}_esp")
                monday_note("Código especie")

            c7, c8, c9 = st.columns(3)
            with c7:
                opts = [""] + get_opciones("Código cultivo")
                formularios[form_key]["Código cultivo"] = st.selectbox("Código cultivo *", opts, key=f"{form_key}_cult")
            with c8:
                ...
                formularios[form_key]["Número comprobante contrato"] = st.selectbox(
                    "Nro. comprobante contrato *", opts_c, index=idx_c, key=f"{form_key}_contrato")
                if idx_c > 0 and contrato_pre:
                    st.markdown(
                        f'''<div style="font-size:0.7rem;color:#3B6D11;background:#f0f7e8;border-radius:4px;
                        padding:2px 7px;display:inline-block;margin-top:-4px">
                        ↑ Monday · <code style="font-size:0.68rem">{contrato_pre}</code></div>''',
                        unsafe_allow_html=True)
            with c9:
                st.write("")
            ...
            with c1:
                opts = [""] + get_opciones("Código socio")
                idx  = next((j for j,o in enumerate(opts) if pre.get("Código socio","") and pre.get("Código socio","") in o), 0)
                formularios[form_key]["Pagador Flete"] = st.selectbox("Pagador flete *", opts, index=idx, key=f"{form_key}_pagador")
                monday_note("Pagador")
            with c2:
                formularios[form_key]["CUIT Pagador Flete"] = st.text_input("CUIT pagador flete", key=f"{form_key}_cuit_pag")
            with c3:
                opts_tf = get_opciones("Tipo de Flete")
                idx_tf  = next((j for j,o in enumerate(opts_tf) if "Tercero" in o), 0)
                formularios[form_key]["Tipo de Flete"] = st.selectbox("Tipo de flete", opts_tf, index=idx_tf, key=f"{form_key}_tipo_flete")

            # ── Pesos origen ──
            sec("Pesos y humedad — origen")
            c1, c2, c3, c4 = st.columns(4)
            with c1:
                formularios[form_key]["Peso Origen Bruto"] = st.number_input("Peso bruto (kg) *", min_value=0, value=45000, step=100, key=f"{form_key}_pb")
            with c2:
                formularios[form_key]["Peso Origen Tara"] = st.number_input("Tara (kg) *", min_value=0, value=15000, step=100, key=f"{form_key}_tara")
            with c3:
            ...
            with c4:
                formularios[form_key]["% Humedad Origen"] = st.number_input("% Humedad origen", min_value=0.0, max_value=40.0, step=0.1, format="%.1f", key=f"{form_key}_hum_o")
            ...
            with c1:
                ...
                formularios[form_key]["Código transportista"] = st.selectbox("Transportista *", opts, key=f"{form_key}_trans")
            with c2:
                opts = [""] + get_opciones("Chofer (CUIT)")
                formularios[form_key]["Chofer (CUIT)"] = st.selectbox("Chofer (CUIT) *", opts, key=f"{form_key}_chofer")
            with c3:
                opts = [""] + get_opciones("Código intermediario flete")
                formularios[form_key]["Código intermediario flete"] = st.selectbox("Intermediario flete", opts, key=f"{form_key}_int")

            c4, c5 = st.columns(2)
            with c4:
                formularios[form_key]["Carta de Porte"] = st.text_input("Carta de porte *", key=f"{form_key}_cpe")
            with c5:
                formularios[form_key]["% Humedad Destino"] = st.number_input("% Humedad destino", min_value=0.0, max_value=40.0, step=0.1, format="%.1f", key=f"{form_key}_hum_d")
            ...
            with c1:
                ...
                formularios[form_key]["Catac"] = st.selectbox("CATAC *", opts, key=f"{form_key}_catac")
            with c2:
                formularios[form_key]["Distancia Planta"] = st.number_input("Distancia planta (km) *", min_value=0, step=1, key=f"{form_key}_dist")
            with c3:
                formularios[form_key]["Tarifa Catac"] = st.number_input("Tarifa CATAC", min_value=0.0, step=0.01, format="%.2f", key=f"{form_key}_tarifa")
            with c4:
                formularios[form_key]["Coeficiente"] = st.number_input("Coeficiente", min_value=0.0, step=0.001, format="%.3f", key=f"{form_key}_coef")

            c5, c6 = st.columns(2)
            with c5:
                formularios[form_key]["Aforado"] = st.number_input("Aforado", min_value=0.0, step=0.1, key=f"{form_key}_aforado")
            with c6:
                opts = [""] + get_opciones("Código destino")
                formularios[form_key]["Código destino"] = st.selectbox("Código destino *", opts, key=f"{form_key}_dest")
            ...
            with c1:
                ...
                formularios[form_key]["Tipo CPE"] = st.selectbox("Tipo CPE", opts, index=idx, key=f"{form_key}_tipo_cpe")
            with c2:
                opts = [""] + get_opciones("Numerador CPE")
                formularios[form_key]["Numerador CPE"] = st.selectbox("Numerador CPE", opts, key=f"{form_key}_num_cpe")

            # ── Opcionales ──
            st.markdown(
                '''<div style="font-size:0.65rem;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;
                color:#888;border-bottom:1px dashed #ddd;padding-bottom:6px;margin:18px 0 4px">
                Campos opcionales</div>''', unsafe_allow_html=True)
            with st.expander("Depósitos, embolsado y extracción"):
                st.caption("Completar solo si corresponde al tipo de operación")
                cd1, cd2 = st.columns(2)
                with cd1:
                    opts = [""] + get_opciones("Código depósito")
                    formularios[form_key]["Código depósito origen"] = st.selectbox("Depósito origen (solo extracción)", opts, key=f"{form_key}_dep_o")
                with cd2:
                    opts = [""] + get_opciones("Código depósito")
                    formularios[form_key]["Código depósito destino"] = st.selectbox("Depósito destino (solo embolsado/rechazo)", opts, key=f"{form_key}_dep_d")
                st.markdown("---")
                ce1, ce2, ce3 = st.columns(3)
                with ce1:
                    opts = [""] + get_opciones("Código embolsador")
                    formularios[form_key]["Código embolsador"] = st.selectbox("Embolsador", opts, key=f"{form_key}_emb")
                with ce2:
                    formularios[form_key]["Precio Embolsador por TN"] = st.number_input("Precio embolsador / TN", min_value=0.0, step=0.01, key=f"{form_key}_precio_emb")
                with ce3:
                    opts = [""] + get_opciones("Código extractor")
                    formularios[form_key]["Código extractor"] = st.selectbox("Extractor", opts, key=f"{form_key}_ext")
                formularios[form_key]["Precio Extractor por TN"] = st.number_input("Precio extractor / TN", min_value=0.0, step=0.01, key=f"{form_key}_precio_ext")

            # ── Guardar ──
            st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
            c_save, c_info = st.columns([1, 3])
            with c_save:
                if st.button("Guardar CTG", key=f"save_{form_key}", type="primary", use_container_width=True):
                    campos_req = [
                    ...
                    if all(c for c in campos_req):
                        ...
                        st.success("CTG guardado correctamente.")
                        st.rerun()
                    else:
                        st.error("Completá todos los campos obligatorios (*) antes de guardar.")
            with c_info:
                if completado:
